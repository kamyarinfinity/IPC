...
//
// Modified by Kamyar Allahverdi (June 2018)
//
//////////////////////////////////////////////////////////////////////////////
...
namespace interprocess {
  ...
  public:
    ...
    /// Read some data from the named pipe asynchronously.
    /**
     * This function is used to read data to the named pipe.  The
     * function call will block until one or more bytes of data have
     * been read successfully, or until an error occurs.
     ...
     * @param buffer A buffer into which the data will be read.
     *
     * @return A future that contains a tuple of read buffer and its length
     *
     * @throws boost::system::system_error Thrown on failure.
     *
     * @note The read operation may not completely fill the
     * buffer.
     */
    std::future<std::tuple<char*,int>> async_read(char *buffer, const int length) {
      return std::async(std::launch::async,[this,buffer,length]{
        _pimpl->read(buffer, length);
        return std::make_tuple(buffer,length);
      });
    }
     ...
     * buffer.
     */
    std::future<int> async_write(const char *buffer, const int length) {
      return std::async([this,buffer,length]{
        _pimpl->write(buffer, length);
        return length;
      });
    }
  ...
  public:
     ...
     * is made.
     */
    named_pipe_server(const std::string &name):name(name),
    ...
    //    fprintf(stderr, "Removing %s",name.c_str());

    //     if(!remove(path(name.c_str())))
    //       fprintf(stderr,"ERR: Most likey file is still being used by client.");
    //  }
